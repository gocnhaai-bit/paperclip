...
export function MetricCard({ icon: Icon, value, label, description, to, onClick }: MetricCardProps) {
  ...
  const inner = (
    <div className={`h-full rounded-lg border border-border bg-card px-4 py-4 shadow-sm transition-colors sm:px-5 sm:py-5${isClickable ? " cursor-pointer hover:border-primary/35 hover:bg-accent/50" : ""}`}>

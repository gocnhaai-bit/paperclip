...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Paperclip for SMIT", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Paperclip for SMIT",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Paperclip for SMIT");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Paperclip for SMIT");
    expect(buildDocumentTitle([], null)).toBe("Paperclip for SMIT");
  });
});

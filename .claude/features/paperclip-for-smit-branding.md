...
## Warm Workspace host checkpoint — 2026-09-15

The host continuation now has a final local browser matrix of 847/847 passing on the current frozen Storybook build. AppDetail connection-refresh recovery, populated Gateway Apps & tools/client snippets/Edit Cancel, Routines date-window and denied trigger/revision actions, and Skill Studio run/policy denial are covered with typed read-only fixtures across the requested responsive/theme combinations. This evidence is local UI behavior only; OAuth, live mutations, authenticated backend callbacks, native 200% zoom, gated experimental data and the approved A2 asset manifest remain unverified or blocked. No Agent Studio source/runtime integration was added.
...
Task verification now includes a native browser-history transition: the real attachment link replaces the iframe document, browser Back returns to the same task, and the plan panel can still open. A long-content mobile task opens the production Properties sheet and measures every property-value container inside the native viewport before Escape/focus return. The desktop More task actions menu opens by keyboard; a fixture-denied Hide request surfaces “Task update failed” and rolls back the optimistic hidden state. The existing full-shell layout is reused because prior panel/scroll/deep-link repairs remain valid; no task action succeeds. Playwright Chromium did not apply the macOS browser-zoom shortcut (`innerWidth` remained 1440 in both attempts), so native 200% zoom remains explicitly unverified rather than being replaced by CSS zoom or page-scale emulation.
...
Routine previews now carry three typed run outcomes (failed schedule, succeeded manual, skipped overlap), two append-only revisions, and routine/trigger/run activity records. The production Runs owner verifies real source/status filters and Clear all across four viewports/light-dark; trigger creation UI verifies invalid custom cron feedback, disabled submission, recovery to a valid expression, and Cancel without creating a trigger. Streamlined Activity, Version history comparison, Delivery next-fire policy preview, and routine-scoped Audit Activity/Runs mount their actual owners; Audit retains the deliberate split where immutable run/activity history lives outside routine configuration. The native layouts are reused because they already separate editable setup from operational history. No routine, trigger, revision, or run mutation is submitted.
...
PluginSettings Status now reports failed runtime-dashboard, health, and recent-log reads instead of silently substituting “diagnostics unavailable” or omitting the section. Cached health/dashboard/log content remains visible alongside its corresponding alert when React Query retains prior data. A typed ready-plugin preview exposes worker/job/webhook/log/health data and a separate three-error state across four viewports/light-dark; no plugin runtime or configuration mutation occurs. Environment previews now mount populated local/SSH/sandbox rows, the selected default, a real edit→Cancel route, and a create form driven by a run-capable Daytona capability/schema. The native settings layouts are reused because they already separate default selection, provider metadata, create/edit forms and capability-derived fields; no environment probe/default/save/delete is submitted.
...
SkillStudio test-run detail reads now preserve the distinction between an unavailable read and a missing run: initial failures announce the server error, while cached details remain visible with an alert after a background failure. No test execution or run mutation contract changed.
...
Apps Services now retains cached connected-service rows after a failed background refresh, announces the actual read error and offers explicit Refresh services/Try again controls. A typed one-failure preview covers the retry surface without connecting or disconnecting anything; the final selected Services/Studio recovery/detail browser run passes28/28. Service permissions and mutation handlers are unchanged.

Apps gateway and access-profile previews now mount populated native host routes in the existing full shell. Gateway Overview, token history, redacted Activity, and Advanced configuration use one typed local gateway/profile/application/tool fixture; token rows expose only the masked prefix, activity arguments/results contain redacted metadata, and archive confirmation is opened then cancelled without a mutation. Access-profile detail resolves allowed tools and an agent assignment, links into the populated edit wizard, and the edit route can be cancelled back to the fixture route. The existing layouts are reused because they already separate scope, apps, credentials metadata, audit, raw configuration, allowed tools, assignments, and default behavior; no new visual primitive or production API/schema/permission change was needed. Browser verification covers all six routes at 1440×900, 1280×800, 768×1024, and 390×844 in light/dark, including row expansion, masked-token assertions, navigation, Cancel, and body-width checks. No token, gateway, profile, assignment, or OAuth action is submitted.
